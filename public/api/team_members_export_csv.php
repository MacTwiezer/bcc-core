<?php
// team_members.php'nin "CSV indir" ikonu — view_export_csv.php ile AYNI desen
// (UTF-8 BOM, ";" ayırıcı, bcc_csv_injection_guard(), log_audit) farklı bir veri
// kaynağına uygulanmış hâli. Salt-okunur işlem (viewer'a da açık) —
// require_role('editor') YOK, team_members.php'nin kendisi de 'viewer' ile açık.
...
// biçimi (HTML tablo / CSV) aynı satır kümesini paylaşıyor.
...
header('Content-Disposition: attachment; filename="' . $fileName . '_uyeler.csv"');
header('Cache-Control: no-store');

$out = fopen('php://output', 'w');

// UTF-8 BOM — view_export_csv.php ile AYNI gerekçe (Excel Türkçe karakterleri
// BOM olmadan Windows-1254 varsayıp bozuyor).
fwrite($out, "\xEF\xBB\xBF");

fputcsv($out, array('İsim', 'E-posta', 'Rol', 'Ekleyen', 'Eklenme tarihi'), ';');
...
log_audit('team_member.export_csv', 'team', $teamId, array('member_count' => count($members)), $teamId);
